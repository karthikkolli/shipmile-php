...
class ApiRequestor
{
    ...
    private function _requestRaw($method, $url, $params, $headers)
    {
        ...
        if (!$myApiKey) {
            $myApiKey = Shipmile::$apiKey;
        }

        if (!$myApiKey) {
            $msg = 'No API key provided.  (HINT: set your API key using '
                . '"Shipmile::setApiKey(<API-KEY>)".  You can generate API keys from '
                . 'the Shipmile web interface.  See https://shipmile.com/api for '
                . 'details, or email support@shipmile.com if you have any questions.';
        ...
        $uname = php_uname();
        $ua = array(
            'bindings_version' => Shipmile::VERSION,
            'lang' => 'php',
        ...
        $defaultHeaders = array(
            'X-Shipmile-Client-User-Agent' => json_encode($ua),
            'User-Agent' => 'Shipmile/v1 PhpBindings/' . Shipmile::VERSION,
            'Authorization' => 'Bearer ' . $myApiKey,
        );
        if (Shipmile::$apiVersion) {
            $defaultHeaders['Shipmile-Version'] = Shipmile::$apiVersion;
        }
        $hasFile = false;
    ...
    private function _curlRequest($method, $absUrl, $headers, $params, $hasFile)
    {
        ...
        if ($errno == CURLE_SSL_CACERT ||
            ...
            array_push(
                $headers,
                'X-Shipmile-Client-Info: {"ca":"using Shipmile-supplied CA bundle"}'
            );
            $cert = $this->caBundle();
    ...
    public function handleCurlError($errno, $message)
    {
        ...
        switch ($errno) {
            ...
            case CURLE_OPERATION_TIMEOUTED:
                $msg = "Could not connect to Shipmile ($apiBase).  Please check your "
                    . "internet connection and try again.  If this problem persists, "
                    . "you should check Shipmile's service status at "
                    . "https://twitter.com/shipmilestatus, or";
                break;
            case CURLE_SSL_CACERT:
            case CURLE_SSL_PEER_CERTIFICATE:
                $msg = "Could not verify Shipmile's SSL certificate.  Please make sure "
                    . "that your network is not intercepting certificates.  "
                    . "(Try going to $apiBase in your browser.)  "
                    . "If this problem persists,";
                break;
            default:
                $msg = "Unexpected error communicating with Shipmile.  "
                    . "If this problem persists,";
        }
        $msg .= " let us know at support@shipmile.com.";
     ...
     * communicate with Shipmile, merely that the first attempt to does not use a
     * revoked certificate.
     *
    ...
    private function checkSslCert($url)
    {
        if (!function_exists('stream_context_get_params') ||
            ...
            error_log(
                ...
                'certificates Shipmile cannot guarantee that the server has a ' .
        ...
        if (($errno !== 0 && $errno !== null) || $result === false) {
            throw new Error\ApiConnection(
                'Could not connect to Shipmile (' . $url . ').  Please check your ' .
                'internet connection and try again.  If this problem persists, ' .
                'you should check Shipmile\'s service status at ' .
                'https://twitter.com/shipmilestatus. Reason was: ' . $errstr
            );
        }
        ...
        if (self::isBlackListed($pemCert)) {
            throw new Error\ApiConnection(
                ...
                'data to that server.  Please email support@shipmile.com if you need ' .
